...
static int GetPixelJulia(double Zx, double Zy)
 {
...
/*  mult = strtod(argv[6], &endptr);   */      /* iteration multiplier */
  ...
  /* check for logarithmic value allocation (algorithm 2) and 
   * setup log variable */
  if (algo == 2)
  {
    logdiv = (log (256 * abs(mult))) / 255;
  };
  ...
  FILE *gnuplot_out = fopen("gnuplot.dat", "w+");

  int prv_i = 0;
  if (algo == 1)
  {
    for(lj = lbj, i = 0; lj <= ubj, i < ysize; lj = lj + increm, i++)
    {
      clock_t start = clock();
      
      for(lr = lbr, j = 0; lr <= ubr, j < xsize; lr = lr + increm, j++)
      {
	      index = getPixel(lr, lj);
        
        index = index % 256;
        all_dem_pixels[i * xsize + j] = index;
      };
      clock_t end = clock();
      float seconds = (float)(end - start) / CLOCKS_PER_SEC;
      fprintf(gnuplot_out, "%d %f\n", prv_i, seconds);
      prv_i++;
    };
  };
